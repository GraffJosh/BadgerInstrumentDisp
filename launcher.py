...
try:
    ...
    print("memory error loading png lib ", e)
    gc.collect()
    png = pngdec.PNG(display.display)
...
drivers = [x[:-3] for x in os.listdir("/drivers") if x.endswith(".py")]
# drivers += [x["name"].split("/").pop() for x in mqtt.topics]
# Approximate center lines for buttons A, B and C
centers = (41, 147, 253)

MAX_PAGE = math.ceil(len(drivers) / 3)
...
def render():
    ...
    draw_disk_usage(90)
    display.set_pen(15)
    display.text("Aion", 4, 4, WIDTH, 1.0)
...
def launch_example(index):
    ...
    file = drivers[(state["page"] * 3) + index]
    file = f"{APP_DIR}/{file}"
